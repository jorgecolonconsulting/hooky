...
trait HooksTrait
{
    ...
    public function __call($method, $arguments)
    {
        ...
        $callableRegistrationTokens = [
            '^(after)(.*)', '^(before)(.*)', '^(onceAfter)(.*)', '^(onceBefore)(.*)'
        ];
        ...
        if (($matches = $this->matchesAny($callableRegistrationTokens, $method))!= false) {
            if (! empty($matches[2]) ) $this->methodExists($matches[2]);

            $targetMethod = lcfirst($matches[2]);

            $this->methodNotRestricted($targetMethod, $method);

            $this->hooks[$method][] = $arguments[0];
        ...
        if (($matches = $this->matchesAny($callableInvokeTokens, $method)) != false) {
            if (strpos($method, 'callAfter') !== false) {
                $return = $this->callAfterMethodHooks($arguments[0], $arguments[1], $extraArgs);
            }

            if (strpos($method, 'callBefore') !== false) {
                $return = $this->callBeforeMethodHooks($arguments[0], $arguments[1], $extraArgs);
            }
        }

        if (($matches = $this->matchesAny($callableInvokeOnceTokens, $method)) != false) {
            if (strpos($method, 'callOnceAfter') !== false) {
                $return = $this->callOnceAfterMethodHooks($arguments[0], $arguments[1], $extraArgs);
            }

            if (strpos($method, 'callOnceBefore') !== false) {
                $return = $this->callOnceBeforeMethodHooks($arguments[0], $arguments[1], $extraArgs);
            }
        }
     ...
     * @return mixed
     */
    protected function callOnceBeforeAllHooks($classInstance, $method, array $args)
    {
        $method = $this->getCalledMethod($method);
     ...
     */
    protected function callOnceAfterAllHooks($classInstance, $method, array $args)
    {
        $method = $this->getCalledMethod($method);
    ...
    /**
     * @param $classInstance
     * @param callable $callable parameters sent to callable: ($classInstance, $method [, $args]) if $method string is
     *                           sent internally, ($classInstance [, $args]) if $method string is not sent
     * @param array    $args
     * @param string   $method
     *
     * @return mixed
    ...
    protected function callCallable(
        $classInstance, callable $callable, array $args = [], $method = null, $includeMethodParameter = true
    )
    {
        $this->checkClosure($classInstance, $callable, $args, $method, $includeMethodParameter);
    ...
    )
    {
        ...
        if ($args && $method && self::$debugMode) {
            ...
            foreach ($originalMethodReflectionParameters as $parameter) {
                $originalMethodParameters[] = $parameter->getName();
            }
            ...
            $parameterMeta = array_map(
                function($originalMethodParameter, $callableParameter) {
                    return ['original' => $originalMethodParameter, 'callable' => $callableParameter];
                },
                $originalMethodParameters,
                $callableParameters
            );
            ...
            if ($parameterDiff) {
                ...
                foreach ($parameterDiff as $key => $parameter) {
                    ...
                    if ($parameter['original'] === null && $parameter['callable'] !== null) {
                        ...
                        $errorBuffer[] = "Callable argument {$callablePosition} exists in the original {$method}() method
                            as argument {$originalPosition} but is omitted in the callable";
                    } elseif ($parameter['original'] !== $parameter['callable']) {
                        $errorBuffer[] = "Callable argument {$callablePosition} '{$parameter['callable']}' is named
                            '{$parameter['original']}' in the original {$method}() method as argument {$originalPosition}";
                    }
                }
